...
ConstraintProximity findClosestElement(const ConstraintProximity & T, BaseGeometry * geo) {
    ConstraintProximity min_pinfo(geo,-1);
    double minDist = std::numeric_limits<double>::max();

    std::unique_ptr<BaseConstraintIterator> iterator = geo->getIterator(T);

    defaulttype::Vector3 P = T.getPosition();
...
ConstraintProximity CollisionAlgorithm::findClosestProximity(const ConstraintProximity &T, BaseGeometry * geo) {
//    AABBDecorator * aabb_decorator;
//    geo->getContext()->get(aabb_decorator);

//    ConstraintProximity res;
//    if (aabb_decorator == NULL) res = findClosestElement(T.getPosition(),geo);
//    else res =
