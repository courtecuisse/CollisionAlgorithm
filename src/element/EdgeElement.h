...
public:
    ...
    : ConstraintElement(geo->getState(),
                        ControlPoint<2>(createProximity(1,0),createProximity(0,1)),
                        createProximity(0.5,0.5))
    , m_geometry(geo) {
        m_eid = eid;
    ...
    ConstraintProximityPtr createProximity(double f1,double f2);
    ...
    ConstraintProximityPtr project(Vector3 P) {
        ...
        return createProximity(fact_u,fact_v);
    }
...
inline ConstraintProximityPtr EdgeElement::createProximity(double f1,double f2) {
    return std::make_shared<EdgeProximity>(this,f1,f2);
}
