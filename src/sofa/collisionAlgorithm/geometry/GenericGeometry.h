...
public:
    ...
    void normalize(PROXIMITYDATA & result) const {
        //Normalize (sum = 1)
        double sum = 0.0;
        for (Index i=0;i<result.m_prox.size();i++) sum += result.m_prox[i].second;
        if (sum == 0.0) return;
        for (Index j=0;j<result.m_prox.size();j++) result.m_prox[j].second *= 1.0/sum;
    }
    ...
    inline PROXIMITYDATA project(defaulttype::Vector3 Q, Index eid) const {
        ...
        std::vector<bool> usePoints(Element::size(),true);
        ...
        while (it< d_nonlin_max_it.getValue()) {
            ...
            for (Index i=0;i<usePoints.size();i++) {
                if (usePoints[i]) {
                    Coord dir = x[result.m_prox[i].first] - P;
                    normals.push_back(std::pair<Index,Coord>(i,dir));
                }
            }

            if (normals.size() <= 1) break;

            Index JLin = normals.size();
...
//            std::cout << "dx=\n" << dx << std::endl;
            ...
            for (Index i=0;i<Element::size();i++) {
                ...
                if (fact < 0) {
                    for (Index j=0;j<new_result.m_prox.size();j++) {
            ...
            double real_dx = 0.0;
            for (Index i=0;i<Element::size();i++) real_dx += std::pow((new_result.m_prox[i].second - result.m_prox[i].second),2);
...
//        double sum = 0.0;
//        for (Index i=0;i<m_fact.size();i++) sum += m_fact[i];
