...
class IntersectOperation : public GenericOperation2<IntersectOperation, std::function<std::pair<BaseElement::SPtr,BaseElement::SPtr> (BaseElement* , BaseElement*)> > {
public:

    using Inherit = GenericOperation2;

    Inherit::FUNC getDefault() const override {
        return [=](BaseElement* e1, BaseElement* e2) -> std::pair<BaseElement::SPtr,BaseElement::SPtr> {
