...
class DefaultTetrahedronProximity : public TBaseProximity<DataTypes> {
public:
    ...
    DefaultTetrahedronProximity(State * state, unsigned p0,unsigned p1, unsigned p2, unsigned p3, double f0,double f1,double f2,double f3)
    : m_state(state), m_p0(p0), m_p1(p1), m_p2(p2), m_p3(p3), m_f0(f0), m_f1(f1), m_f2(f2), m_f3(f3){}
    ...
    sofa::type::Vector3 getPosition(core::VecCoordId v = core::VecCoordId::position()) const {
        ...
        return pos[m_p0] * m_f0 + pos[m_p1] * m_f1 + pos[m_p2] * m_f2 + pos[m_p3] * m_f3;
    }
